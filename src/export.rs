...
fn balance_el(side: &str, cents: i64) -> String {
    ...
    if cents >= 0 {
        ...
        format!("        <{side}CreditBalance>{}</{side}CreditBalance>", format_amount(-cents))
    }
}
...
fn to_boekingen(postings: &[(i64, String)]) -> Vec<(String, String, i64)> {
    let mut debits: Vec<(String, i64)> = postings.iter()
        .filter(|(_, c)| c.starts_with('+') || !c.starts_with('-'))
        .filter_map(|(amt, code)| if *amt > 0 { Some((code.clone(), *amt)) } else { None })
        .collect();
    let mut credits: Vec<(String, i64)> = postings.iter()
        .filter_map(|(amt, code)| if *amt < 0 { Some((code.clone(), -amt)) } else { None })
        .collect();
...
fn build_xaf_40(db: &Connection, year: &str, out: &str, actor: &str, dry_run: bool) -> Result<Value> {
    if year.len() != 4 || !year.chars().all(|c| c.is_ascii_digit()) {
        return Err(export_error("INVALID_YEAR", format!("year '{year}' must be YYYY")));
    }
    ...
    let accounts = {
        let mut stmt = db.prepare("SELECT code, name, type FROM accounts ORDER BY code").map_err(sql_err)?;
        let rows: Vec<(String, String, String)> = stmt.query_map([], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).map_err(sql_err)?.filter_map(|r| r.ok()).collect();
        rows
    };
    ...
    let entries = {
        ...
        let rows: Vec<(i64, String, String, String)> = stmt.query_map(rusqlite::params![fy_from, fy_to], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?))).map_err(sql_err)?.filter_map(|r| r.ok()).collect();
        rows
    };
    ...
    let postings = {
        ...
        let rows: Vec<(i64, i64, String)> = stmt.query_map(rusqlite::params![fy_from, fy_to], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).map_err(sql_err)?.filter_map(|r| r.ok()).collect();
        rows
    };
    ...
    let posted: Vec<&(i64, String, String, String)> = entries.iter().filter(|e| e.3 == "posted").collect();
    if posted.is_empty() {
        return Err(export_error("EXPORT_EMPTY_YEAR", format!("no posted entries in fiscal year {year}")));
    }

    let version = env!("CARGO_PKG_VERSION");

    if dry_run {
        return Ok(json!({"ok": true, "path": out, "year": year, "dryRun": true,
    ...
    for (eid, _date, desc, _) in &posted {
        ...
        let d = if desc.is_empty() { format!("Boeking {eid}") } else { desc.clone() };
    ...
    record(db, RecordArgs { actor, action: "export.xaf", command: Some("export xaf"), args: Some(json!({"year": year, "out": out})), outcome: "ok", entry_ids: vec![] })?;
    Ok(json!({"ok": true, "path": out, "year": year, "company": {"name": company["name"], "registration_id": company["registration_id"]}, "rekeningen": accounts.len(), "mutaties": mutatie_ids.len()}))
}

/// Export FAIA 2.01 reduced B (Luxembourg).
fn build_faia_b(db: &Connection, year: &str, out: &str, actor: &str, dry_run: bool) -> Result<Value> {
    if year.len() != 4 || !year.chars().all(|c| c.is_ascii_digit()) {
        return Err(export_error("INVALID_YEAR", format!("year '{year}' must be YYYY")));
    }
    let company = db.query_row("SELECT * FROM company WHERE id = 1", [], |r| {
    ...
    let accounts: Vec<(String, String, String)> = db.prepare("SELECT code, name, type FROM accounts ORDER BY code").map_err(sql_err)?
    ...
    for (pid, eid, amt, code) in &postings {
        by_entry.entry(*eid).or_default().push((*pid, *amt, code.clone()));
    }

    let posted: Vec<_> = entries.iter().filter(|e| e.3 == "posted").collect();
    if posted.is_empty() {
        return Err(export_error("EXPORT_EMPTY_YEAR", format!("no posted entries in {year}")));
    }
    ...
    let balances: Vec<(String, i64, i64)> = db
        ...
        .filter_map(|r| r.ok())
        .collect();
    let bal_map: HashMap<String, (i64, i64)> = balances.into_iter().map(|(k, v1, v2)| (k, (v1, v2))).collect();
    ...
    xml.push_str(&format!("      <RegistrationNumber>{}</RegistrationNumber>\n", esc(company["registration_id"].as_str().unwrap_or(""))));
    ...
    xml.push_str("    </Company>\n");
    xml.push_str(&format!("    <DefaultCurrencyCode>{}</DefaultCurrencyCode>\n", esc(company["base_currency"].as_str().unwrap_or("EUR"))));
        ...
        if let Some(codes) = profile["tax"]["codes"].as_array() {
            ...
            for c in codes {
                ...
                if c["type"] == "standard" {
                    ...
                    xml.push_str(&format!("          <TaxPercentage>{:.2}</TaxPercentage>\n", rate));
                }
    ...
    for (_, amt, _, _) in &all_lines {
        if *amt > 0 { total_debit += amt; } else { total_credit += -amt; }
    }

    xml.push_str("  <GeneralLedgerEntries>\n");
    xml.push_str(&format!("    <NumberOfEntries>{}</NumberOfEntries>\n", posted.len()));
    xml.push_str(&format!("    <TotalDebit>{}</TotalDebit>\n    <TotalCredit>{}</TotalCredit>\n", format_amount(total_debit), format_amount(total_credit)));
    ...
    for (eid, date, desc, _state, posted_at) in &posted {
        ...
        let mm: i64 = if d.len() >= 7 { d[5..7].parse().unwrap_or(1) } else { 1 };
        let posted_date = posted_at.as_deref().map(|p| p[..10.min(p.len())].to_string()).unwrap_or_else(|| d.to_string());
        let desc_str = if desc.is_empty() { format!("Écriture {eid}") } else { desc.clone() };
        ...
        for (pid, amt, code) in &rows {
            ...
            if *amt > 0 {
                xml.push_str(&format!("          <DebitAmount><Amount>{}</Amount></DebitAmount>\n", format_amount(*amt)));
            } else if *amt < 0 {
                xml.push_str(&format!("          <CreditAmount><Amount>{}</Amount></CreditAmount>\n", format_amount(-amt)));
            } else {
    ...
    fs::write(out, &xml).map_err(|e| export_error("WRITE_FAILED", e.to_string()))?;
    record(db, RecordArgs { actor, action: "export.xaf", command: Some("export xaf"), args: Some(json!({"year": year, "out": out})), outcome: "ok", entry_ids: vec![] })?;
    Ok(json!({"ok": true, "path": out, "year": year, "company": {"name": company["name"], "registration_id": company["registration_id"]}, "rekeningen": accounts.len(), "mutaties": if all_lines.is_empty() { 0 } else { posted.len() }}))
}

pub fn export_xaf(db: &Connection, year: &str, out: &str, actor: &str, dry_run: bool) -> Result<Value> {
    let profile = resolve_profile(db)?;
    let audit_format = profile["documents"]["auditFile"].as_str().unwrap_or("xaf-auditfile-4.0");
    match audit_format {
        "xaf-auditfile-4.0" => build_xaf_40(db, year, out, actor, dry_run),
        "faia-2.01-reduced-b" => build_faia_b(db, year, out, actor, dry_run),
        _ => Err(export_error("FORMAT_NOT_SUPPORTED", format!("audit file format '{audit_format}' has no builder"))),
    }
}
...
mod tests {
    ...
    fn test_db() -> Connection {
        ...
        d.execute("INSERT INTO company (name, registration_id) VALUES ('ExportCo', '12345')", []).unwrap();
    ...
    fn export_xaf_dry_run() {
        ...
        let e = crate::entries::create_entry(&d, crate::entries::CreateEntry {
            date: "2026-01-15", description: "Test",
            postings: vec![
                crate::entries::PostingSpec { code: "8000".into(), amount_cents: 10000, cost_center_code: None },
                crate::entries::PostingSpec { code: "1000".into(), amount_cents: -10000, cost_center_code: None },
            ],
