...
fn unit_code(unit: &str) -> &str {
    UNIT_CODE_MAP.iter().find(|(k, _)| *k == unit).map(|(_, v)| *v).unwrap_or("C62")
}

fn address_block(party_name: &str, p: &Value, tax_id: Option<&str>, scheme_id: &str, default_country: &str) -> String {
    let postal = p.get("postalCode").or_else(|| p.get("postal_code")).and_then(|v| v.as_str()).unwrap_or("");
    let endpoint = p.get("registration_id").and_then(|v| v.as_str()).map(|rid| {
        format!("\n        <cbc:EndpointID schemeID=\"{scheme_id}\">{}</cbc:EndpointID>", esc(rid))
    }).unwrap_or_default();
    ...
    let reg_id = p.get("registration_id").and_then(|v| v.as_str()).map(|rid| {
    ...
    format!("\n        <cac:Party>{endpoint}
        ...
        esc(postal),
        esc(p.get("country").and_then(|v| v.as_str()).unwrap_or(default_country)),
        esc(party_name))
}

fn buyer_scheme_id<'a>(profile: &'a Value, contact: &Value) -> &'a str {
    let seller_scheme = profile.get("identifiers").and_then(|i| i.get("peppolSchemeId")).and_then(|v| v.as_str()).unwrap_or("9944");
    let buyer_country = contact.get("country").and_then(|v| v.as_str())
        .unwrap_or(profile.get("meta").and_then(|m| m.get("country")).and_then(|v| v.as_str()).unwrap_or("NL"));
    let seller_country = profile.get("meta").and_then(|m| m.get("country")).and_then(|v| v.as_str()).unwrap_or("NL");
    if buyer_country.to_uppercase() == seller_country.to_uppercase() { return seller_scheme; }
...
pub fn invoice_to_ubl(db: &Connection, invoice: &Value) -> Result<String, BukioError> {
    ...
    let eformat = profile.get("documents").and_then(|d| d.get("eInvoicing")).and_then(|v| v.as_str()).unwrap_or("");
    if eformat != "peppol-bis-3.0" {
        return Err(ubl_error("FORMAT_NOT_SUPPORTED", format!("e-invoicing format '{eformat}' has no builder")));
    }
    build_peppol_bis30(db, invoice, &profile)
}

fn build_peppol_bis30(db: &Connection, invoice: &Value, profile: &Value) -> Result<String, BukioError> {
    ...
    let is_credit = invoice.get("invoiceType").or_else(|| invoice.get("invoice_type")).and_then(|v| v.as_str()) == Some("credit");
    let type_code = if is_credit { "381" } else { "380" };
    let currency = invoice.get("currency").and_then(|v| v.as_str()).unwrap_or("EUR");
    let contact = invoice.get("contact").unwrap_or(&Value::Null);
    let lines: Vec<Value> = invoice.get("lines").and_then(|l| l.as_array()).map(|a| a.iter().cloned().collect()).unwrap_or_default();
    let discount_type = invoice.get("discountType").or_else(|| invoice.get("discount_type")).and_then(|v| v.as_str());
    let discount_value = invoice.get("discountValue").or_else(|| invoice.get("discount_value")).and_then(|v| v.as_i64());

    // Credit note billing reference
    let credit_billing_ref = if is_credit {
        invoice.get("creditForInvoiceId").or_else(|| invoice.get("credit_for_invoice_id"))
            .and_then(|v| v.as_i64())
            .and_then(|fid| {
                db.query_row("SELECT invoice_number FROM invoices WHERE id = ?1", [fid], |r| r.get::<_, Option<String>>(0)).ok().flatten()
            })
    } else { None };
    ...
    let net_before_cents = totals.get("net_before_cents").or_else(|| totals.get("netBeforeCents")).and_then(|v| v.as_i64()).unwrap_or(net_cents + discount_cents);
    let groups = totals.get("groups").and_then(|g| g.as_array()).cloned().unwrap_or_default();

    // Tax subtotals
    let mut subtotal_map: HashMap<String, (String, i64, i64, i64)> = HashMap::new();
    for g in &groups {
        let discounted_net = g.get("discountedNet").or_else(|| g.get("discounted_net")).and_then(|v| v.as_i64()).unwrap_or(0);
        if discounted_net == 0 { continue; }
        let code = g.get("code").and_then(|v| v.as_str()).unwrap_or("");
        let rate_bp = g.get("rateBp").or_else(|| g.get("rate_bp")).and_then(|v| v.as_i64()).unwrap_or(0);
        let cat = vat_category(code);
        let key = format!("{cat}|{rate_bp}");
        let e = subtotal_map.entry(key).or_insert_with(|| (cat.to_string(), rate_bp, 0, 0));
        e.2 += discounted_net;
        e.3 += g.get("vat").and_then(|v| v.as_i64()).unwrap_or(0);
    }
    let standard_rate_bp = profile.get("tax").and_then(|t| t.get("standardRateBp")).and_then(|v| v.as_i64()).unwrap_or(2100);
    ...
    // Lines XML
    let seller_scheme = profile.get("identifiers").and_then(|i| i.get("peppolSchemeId")).and_then(|v| v.as_str()).unwrap_or("9944");
    let default_country = profile.get("meta").and_then(|m| m.get("country")).and_then(|v| v.as_str()).unwrap_or("NL");
    let line_tag = if is_credit { "CreditNoteLine" } else { "InvoiceLine" };
    let qty_tag = if is_credit { "CreditNoteLineQuantity" } else { "InvoicedQuantity" };
    ...
    let line_ext_total: i64 = lines.iter().map(|l| {
        let amt = l.get("amountCents").or_else(|| l.get("amount_cents")).and_then(|v| v.as_i64()).unwrap_or(0);
    ...
    let due_date = invoice.get("dueDate").or_else(|| invoice.get("due_date")).and_then(|v| v.as_str());
    let reference = invoice.get("reference").and_then(|v| v.as_str());
    let notes = invoice.get("notes").and_then(|v| v.as_str());
    let inv_number = invoice.get("invoiceNumber").or_else(|| invoice.get("invoice_number")).and_then(|v| v.as_str()).unwrap_or("");
    let inv_date = invoice.get("date").and_then(|v| v.as_str()).unwrap_or("");

    let root_tag = if is_credit { "CreditNote" } else { "Invoice" };
    let root_ns = if is_credit { "urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2" } else { "urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" };
    let type_tag = if is_credit { "CreditNoteTypeCode" } else { "InvoiceTypeCode" };

    let seller_party = address_block(company["name"].as_str().unwrap_or(""), &company, company["tax_id"].as_str(), seller_scheme, default_country);
    let buyer_endpoint = contact.get("kvk").and_then(|v| v.as_str()).map(|kvk| {
        let scheme = buyer_scheme_id(profile, contact);
        format!("\n      <cbc:EndpointID schemeID=\"{scheme}\">{}</cbc:EndpointID>", esc(kvk))
    }).unwrap_or_default();
    let buyer_reg_id = contact.get("kvk").and_then(|v| v.as_str()).map(|kvk| {
    ...
    let doc_allowance = if discount_cents > 0 {
        let pct_val = if net_before_cents > 0 { (discount_cents as f64 / net_before_cents as f64) * 100.0 } else { 0.0 };
        format!("\n  <cac:AllowanceCharge>\n    <cbc:ChargeIndicator>false</cbc:ChargeIndicator>\n    <cbc:AllowanceChargeReasonCode>95</cbc:AllowanceChargeReasonCode>\n    <cbc:Amount currencyID=\"{currency}\">{}</cbc:Amount>\n    <cbc:BaseAmount currencyID=\"{currency}\">{}</cbc:BaseAmount>\n    <cbc:MultiplierFactorNumeric>{:.4}</cbc:MultiplierFactorNumeric>\n  </cac:AllowanceCharge>", money_amount(discount_cents), money_amount(net_before_cents), pct_val)
    } else { String::new() };
    let due_xml = due_date.map(|dd| format!("\n  <cbc:DueDate>{dd}</cbc:DueDate>")).unwrap_or_default();
    let note_xml = notes.map(|n| format!("\n  <cbc:Note>{}</cbc:Note>", esc(n))).unwrap_or_default();
    let ref_xml = reference.map(|r| format!("\n  <cbc:BuyerReference>{}</cbc:BuyerReference>", esc(r))).unwrap_or_default();
