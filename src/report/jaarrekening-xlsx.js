...
export async function renderJaarrekeningXlsx(report, { outPath }) {
  ...
  balans.addRow([`Jaarrekening ${report.year} (${report.model})`, '', '', '']);
  balans.addRow([`${report.company.name} — ${report.as_of}`, '', '', '']);
  balans.addRow([]);
  ...
  for (let i = 0; i < rows; i += 1) {
    ...
    balans.addRow([a?.label ?? '', a ? a.total_cents / 100 : '', p?.label ?? '', p ? p.total_cents / 100 : '']);
  }
  ...
  if (report.pnl) {
    ...
    pnl.addRow([`Winst- en verliesrekening ${report.year}`, '']);
    pnl.addRow([]);
    for (const l of report.pnl.lines) {
      pnl.addRow([l.label, l.total_cents / 100]);
      for (const s of l.sections ?? []) {
        for (const a of s.accounts ?? []) pnl.addRow([`  ${a.name}`, a.amount_cents / 100]);
      }
    }
