...
pub fn parse_line_spec(spec: &str) -> Result<Value> {
    ...
    let (qty_milli, rest) = if let Some(idx) = s.find("x ") {
        ...
        if qty_str.starts_with('-') {
            return Err(invoice_error("INVALID_LINE", format!("line '{spec}': quantity must be positive")));
        }
        let qty_f: f64 = qty_str.parse().map_err(|_| invoice_error("INVALID_LINE", format!("line '{spec}': invalid quantity")))?;
        let q = (qty_f * 1000.0).round() as i64;
        if q < 1 { return Err(invoice_error("INVALID_LINE", format!("line '{spec}': quantity must be positive"))); }
        (q, s[idx + 2..].trim())
    } else {
        (1000, s)
    };
    ...
    if parts.len() < 2 {
        return Err(invoice_error("INVALID_LINE", format!("line '{spec}' must be \"[QTYx] DESCRIPTION @ PRICE [@ VATCODE] [@ -DISCOUNT]\"")));
    }
    ...
    let (discount_type, discount_value, parts) = {
        ...
        if last.starts_with('-') {
            ...
            let num_str = if is_pct { &last[1..last.len()-1] } else { &last[1..] };
            let val = parse_dutch_amount(num_str)
                .ok_or_else(|| invoice_error("INVALID_LINE", format!("line '{spec}': invalid discount")))?;
            let dt = if is_pct { "pct" } else { "amount" };
            let dv = if is_pct { val } else { val };
            (Some(dt.to_string()), Some(dv), parts[..parts.len()-1].to_vec())
        } else {
    ...
    if price_cents <= 0 {
        return Err(invoice_error("INVALID_LINE", format!("line '{spec}': price must be positive")));
    }

    let description = parts[..parts.len()-1].join("@").trim().to_string();
    if description.is_empty() {
        return Err(invoice_error("INVALID_LINE", format!("line '{spec}': description required")));
    }
...
pub fn parse_item_spec(spec: &str) -> Result<Value> {
    ...
    let (item_id, qty_milli) = if let Some(colon_idx) = id_part.find(':') {
        let id: i64 = id_part[..colon_idx].parse()
            .map_err(|_| invoice_error("INVALID_ITEM_SPEC", format!("item spec '{spec}': invalid item id")))?;
        let qty_f: f64 = id_part[colon_idx + 1..].parse()
            .map_err(|_| invoice_error("INVALID_ITEM_SPEC", format!("item spec '{spec}': invalid quantity")))?;
        let q = (qty_f * 1000.0).round() as i64;
        if q < 1 { return Err(invoice_error("INVALID_LINE", format!("item spec '{spec}': quantity must be positive"))); }
        (id, q)
    } else {
        let id: i64 = id_part.parse()
            .map_err(|_| invoice_error("INVALID_ITEM_SPEC", format!("item spec '{spec}': invalid item id")))?;
        (id, 1000)
    };
    ...
    let overrides: Vec<&str> = parts[1..].iter().map(|p| p.trim()).filter(|p| !p.is_empty()).collect();
    let mut consumed = 0;
    if !overrides.is_empty() {
        ...
        if last.starts_with('-') {
            ...
            let num_str = if is_pct { &last[1..last.len()-1] } else { &last[1..] };
            let val = parse_dutch_amount(num_str)
                .ok_or_else(|| invoice_error("INVALID_LINE", format!("item spec '{spec}': invalid discount")))?;
            discount_type = Some(if is_pct { "pct".to_string() } else { "amount".to_string() });
        ...
        let remaining = overrides.len() - consumed;
        if remaining > 0 {
            price_cents = Some(parse_dutch_amount(overrides[remaining - 1])
                .ok_or_else(|| invoice_error("INVALID_LINE", format!("item spec '{spec}': invalid price")))?);
        }
    }
...
pub fn allocate_largest_remainder(total: i64, weights: &[i64]) -> Vec<i64> {
    ...
    if w_sum <= 0 { return vec![0; n]; }
    let exact: Vec<f64> = weights.iter().map(|&w| (w as f64 * total as f64) / w_sum as f64).collect();
    let mut alloc: Vec<i64> = exact.iter().map(|e| *e as i64).collect();
    let mut rest = total - alloc.iter().sum::<i64>();
    let mut order: Vec<(usize, f64)> = exact.iter().enumerate().map(|(i, e)| (i, e - alloc[i] as f64)).collect();
...
pub fn compute_invoice_totals(lines: &[Value], discount_type: Option<&str>, discount_value: Option<i64>) -> Value {
    // Compute line nets
    let line_nets: Vec<Value> = lines.iter().map(|l| {
    ...
    let net_before: i64 = line_nets.iter().map(|l| l["lineNet"].as_i64().unwrap_or(0)).sum();
    let discount_cents = match discount_type {
        Some("pct") => ((net_before as f64 * discount_value.unwrap_or(0) as f64) / 10000.0).round() as i64,
    ...
    for l in &line_nets {
        ...
        if let Some(g) = groups.iter_mut().find(|g| g["key"].as_str() == Some(&key)) {
            ...
            if let Some(arr) = g["lines"].as_array_mut() { arr.push(l.clone()); }
        } else {
            groups.push(json!({
    ...
    let group_weights: Vec<i64> = groups.iter().map(|g| g["net"].as_i64().unwrap_or(0).max(0)).collect();
    ...
    for (i, g) in groups.iter_mut().enumerate() {
        ...
        let vat = ((discounted.abs() as f64 * g["rateBp"].as_i64().unwrap_or(0) as f64) / 10000.0).round() as i64;
        g["vat"] = json!(vat);

        // Distribute group VAT across lines
        let line_weights: Vec<i64> = g["lines"].as_array().unwrap_or(&vec![]).iter()
            .map(|l| l["lineNet"].as_i64().unwrap_or(0).max(0)).collect();
    ...
    let net: i64 = groups.iter().map(|g| g["discountedNet"].as_i64().unwrap_or(0)).sum();
    ...
    for g in &groups {
        ...
        if let Some(b) = breakdown.iter_mut().find(|b| b["rate_bp"] == rate) {
            b["base_cents"] = json!(b["base_cents"].as_i64().unwrap_or(0) + g["discountedNet"].as_i64().unwrap_or(0));
            b["vat_cents"] = json!(b["vat_cents"].as_i64().unwrap_or(0) + v);
        } else {
            breakdown.push(json!({"rate_bp": rate, "base_cents": g["discountedNet"], "vat_cents": v}));
        }
    }
    breakdown.sort_by(|a, b| b["rate_bp"].as_i64().unwrap_or(0).cmp(&a["rate_bp"].as_i64().unwrap_or(0)));
...
pub fn get_invoice(db: &Connection, id: i64) -> Result<Option<Value>> {
    let inv = db.query_row("SELECT * FROM invoices WHERE id = ?1", [id], |r| serialize_invoice_row(r));
    ...
    let contact = inv["contact_id"].as_i64().and_then(|cid| get_contact(db, cid).ok().flatten());
    let t = compute_invoice_totals(&lines, inv["discount_type"].as_str(), inv["discount_value"].as_i64());
    ...
    let paid: i64 = payments.iter().map(|p| p["amount_cents"].as_i64().unwrap_or(0)).sum();
...
pub fn list_invoices(db: &Connection, status: Option<&str>, invoice_type: Option<&str>) -> Result<Vec<Value>> {
    ...
    let rows = stmt.query_map(param_refs.as_slice(), |r| serialize_invoice_row(r)).map_err(sql_err)?;
    ...
    for inv in invoices.iter_mut() {
        ...
        if inv["status"].as_str() == Some("sent") {
            if let Some(due) = inv["due_date"].as_str() {
                let today = crate::dates::today_iso();
                if due < today.as_str() { inv["status"] = json!("overdue"); }
            }
        }
    }
...
fn posting_defaults(db: &Connection) -> Result<Value> {
    ...
    let sales = profile["reporting"]["defaultChart"].as_array()
        .and_then(|arr| arr.iter().find(|a| a["type"] == "income"));
    let sales_code = sales.ok_or_else(|| invoice_error("FORMAT_NOT_SUPPORTED", "profile declares no income account"))?
        ["code"].as_str().ok_or_else(|| invoice_error("FORMAT_NOT_SUPPORTED", "income account has no code"))?;

    let mut vat_liability_code = None;
    if is_vat_enabled(db) {
        let vat_liab = profile["tax"]["accounts"]["ledger"].as_array()
            .and_then(|arr| arr.iter().find(|a| a["type"] == "liability"));
        vat_liability_code = Some(vat_liab.ok_or_else(|| invoice_error("FORMAT_NOT_SUPPORTED", "profile declares no VAT liability account"))?
            ["code"].as_str().ok_or_else(|| invoice_error("FORMAT_NOT_SUPPORTED", "VAT liability account has no code"))?.to_string());
    }
...
pub fn build_invoice_postings(db: &Connection, invoice: &Value) -> Result<Vec<Value>> {
    ...
    let t = compute_invoice_totals(&lines, invoice["discount_type"].as_str(), invoice["discount_value"].as_i64());
    ...
    if vat_on {
        for g in &groups {
            ...
            if dn == 0 { continue; }
            let gl = g["gl"].as_str().or_else(|| pd["salesCode"].as_str()).unwrap_or("8000");
...
pub fn validate_compliance(db: &Connection, invoice: &Value) -> Result<()> {
    let profile = resolve_profile(db)?;
    let rule_key = profile["documents"]["invoiceCompliance"].as_str().unwrap_or("");
    if rule_key.is_empty() {
        return Err(invoice_error("FORMAT_NOT_SUPPORTED", "no invoice compliance rule set for this profile"));
    }
    ...
    if company["name"].as_str().is_none() { missing.push("company name"); }
    let supplier_has_vat = company["vat_module"].as_i64() == Some(1) || company["tax_id"].as_str().is_some();
    if supplier_has_vat && company["tax_id"].as_str().is_none() { missing.push("tax id"); }
    if company["registration_id"].as_str().is_none() { missing.push("registration number"); }
    if company["address"].as_str().is_none() { missing.push("address"); }
    if company["postal_code"].as_str().is_none() { missing.push("postal code"); }
    if company["city"].as_str().is_none() { missing.push("city"); }
    if !missing.is_empty() {
        return Err(invoice_error("SUPPLIER_INCOMPLETE", format!("supplier details missing: {} — set them with init/company update", missing.join(", "))));
    }
    ...
    if let Some(c) = contact {
        if c.get("name").and_then(|v| v.as_str()).is_none()
            ...
            return Err(invoice_error("CUSTOMER_INCOMPLETE", "customer details missing: name, address and city are required"));
        }
    }

    // Reverse charge check
    let lines = invoice["lines"].as_array().cloned().unwrap_or_default();
    let has_reverse = lines.iter().any(|l| {
        matches!(l["vat_code"].as_str(), Some("R") | Some("RE"))
    });
    if has_reverse {
        let contact = invoice["contact"].as_object();
        if let Some(c) = contact {
            if c.get("vat_id").and_then(|v| v.as_str()).is_none() {
                return Err(invoice_error("CUSTOMER_VAT_REQUIRED", "reverse-charge invoice: the customer VAT id is required"));
            }
        }
    }
    Ok(())
...
pub fn create_invoice(
    db: &Connection,
    contact_id: i64, date: &str, due_days: Option<i64>,
    description: Option<&str>, reference: Option<&str>, notes: Option<&str>,
    discount_type: Option<&str>, discount_value: Option<i64>,
    lines_raw: &[Value], actor: &str, dry_run: bool,
) -> Result<Value> {
    let contact = get_contact(db, contact_id)?
        .ok_or_else(|| invoice_error("CONTACT_NOT_FOUND", format!("contact {contact_id} does not exist")))?;

    // Validate date
    if date.len() != 10 || date.as_bytes()[4] != b'-' || date.as_bytes()[7] != b'-' {
        return Err(invoice_error("INVALID_DATE", format!("date '{date}' must be YYYY-MM-DD")));
    }
    ...
    let vat_on = is_vat_enabled(db);
    let vat_codes: std::collections::HashMap<String, Value> = list_vat_codes(db)?.into_iter().filter_map(|c| { let code = c.get("code")?.as_str()?.to_string(); Some((code, c)) }).collect();
    ...
    for (i, spec) in lines_raw.iter().enumerate() {
        ...
        if desc.is_empty() || price <= 0 {
            return Err(invoice_error("INVALID_LINE", format!("line {i}: not parseable")));
        }
        let qty = p["qty_milli"].as_i64().unwrap_or(1000);
        if qty < 1 { return Err(invoice_error("INVALID_LINE", format!("line {i}: quantity must be positive"))); }

        let vat_code = p["vat_code"].as_str().map(String::from);
        if let Some(ref vc) = vat_code {
            if !vat_on { return Err(invoice_error("VAT_MODULE_OFF", "line has a VAT code but the VAT module is off")); }
            if !vat_codes.contains_key(vc) { return Err(invoice_error("VAT_CODE_NOT_FOUND", format!("vat code '{vc}' does not exist"))); }
        }
        let rate_bp = vat_code.as_ref().and_then(|vc| vat_codes.get(vc).and_then(|c| c["rate_bp"].as_i64())).unwrap_or(0);
    ...
    record(db, RecordArgs {
        actor, action: "invoice.create", command: Some("invoice create"),
        args: Some(json!({"contactId": contact_id, "date": date, "lines": parsed_lines.len(), "net": totals["net_cents"], "gross": totals["gross_cents"]})),
...
pub fn finalize_invoice(db: &Connection, id: i64, actor: &str, dry_run: bool) -> Result<Value> {
    ...
    if status != "draft" {
        return Err(invoice_error("ALREADY_FINALIZED", format!("invoice {id} is already {status}")));
    }
    ...
    loop {
        match (|| -> Result<Value> {
            ...
            let contact_name = invoice["contact"].get("name").and_then(|v| v.as_str()).unwrap_or("");
            let desc = format!("Invoice {number} - {contact_name}");

            let entry = crate::entries::create_entry(db, crate::entries::CreateEntry {
                date: year_str, description: &desc,
                postings: postings.iter().map(|p| {
                    crate::entries::PostingSpec {
                        code: p["code"].as_str().unwrap_or("").to_string(),
                        amount_cents: p["amountCents"].as_i64().unwrap_or(0),
                        cost_center_code: p.get("costCenterCode").and_then(|v| v.as_str()).map(String::from),
                    }
                }).collect(),
                source: "invoice", source_ref: Some(&format!("inv:{id}")),
                actor,
            })?;
            ...
            record(db, RecordArgs {
                actor, action: "invoice.finalize", command: Some("invoice finalize"),
                args: Some(json!({"id": id, "invoice_number": number})), outcome: "ok",
                entry_ids: vec![posted.id],
            })?;
            Ok(json!({"invoice": get_invoice(db, id)?, "entry": crate::entries::entry_to_json(&posted)}))
        })() {
...
pub fn credit_invoice(db: &Connection, id: i64, date: Option<&str>, reason: Option<&str>, actor: &str, dry_run: bool) -> Result<Value> {
    ...
    if !["sent", "paid", "overdue"].contains(&status) {
        return Err(invoice_error("NOT_FINALIZED", "the invoice must be finalized before crediting"));
    }
    ...
    let credit = create_invoice(db, contact_id, credit_date, None,
        Some(reason.unwrap_or(&format!("Credit note for {}", original["invoice_number"].as_str().unwrap_or("")))),
        original["reference"].as_str(), None,
        original["discount_type"].as_str(), original["discount_value"].as_i64(),
        &lines, actor, false)?;
    ...
    record(db, RecordArgs {
        actor, action: "invoice.credit", command: Some("invoice credit"),
        args: Some(json!({"id": id, "creditId": credit_id})), outcome: "ok", entry_ids: vec![],
    })?;
    get_invoice(db, credit_id)?.ok_or_else(|| BukioError::new("DB_ERROR", "invoice not found after credit"))
}

pub fn mark_paid(db: &Connection, id: i64, date: &str, amount_cents: i64, method: &str, actor: &str, dry_run: bool) -> Result<Value> {
    let invoice = get_invoice(db, id)?
        .ok_or_else(|| invoice_error("NOT_FOUND", format!("invoice {id} does not exist")))?;
    ...
    if !["sent", "overdue"].contains(&status) {
        return Err(invoice_error("NOT_PAYABLE", format!("invoice {id} is {status}")));
    }
    if amount_cents <= 0 { return Err(invoice_error("INVALID_AMOUNT", "payment amount must be positive")); }
    if date.len() != 10 || date.as_bytes()[4] != b'-' || date.as_bytes()[7] != b'-' {
        return Err(invoice_error("INVALID_DATE", format!("payment date '{date}' must be YYYY-MM-DD")));
    }
    ...
    if paid + amount_cents >= gross {
        db.execute("UPDATE invoices SET status = 'paid' WHERE id = ?1", [id]).map_err(sql_err)?;
    }

    record(db, RecordArgs {
        actor, action: "invoice.pay", command: Some("invoice pay"),
        args: Some(json!({"id": id, "amountCents": amount_cents})), outcome: "ok", entry_ids: vec![],
    })?;
...
mod tests {
    ...
    fn next_invoice_number_sequential() {
        ...
        d.execute("INSERT INTO company (name) VALUES ('TestCo')", []).unwrap();
