...
fn ref_exists(db: &Connection, kind: &str, ref_id: i64) -> Result<bool> {
    let table = if kind == "invoice" { "invoices" } else { "journal_entries" };
...
pub fn add_attachment(
    db: &Connection, kind: &str, ref_id: i64, file_path: &str,
    note: Option<&str>, store: &str, actor: &str, dry_run: bool,
) -> Result<Value> {
    if !["invoice", "entry"].contains(&kind) {
        return Err(attachment_error("INVALID_KIND", format!("attachment kind must be 'invoice' or 'entry', got '{kind}'")));
    }
    if ref_id <= 0 {
        return Err(attachment_error("REF_REQUIRED", "pass exactly one of --invoice <id> or --entry <id>"));
    }
    if !["db", "file"].contains(&store) {
        return Err(attachment_error("INVALID_STORE", format!("attachment store must be 'db' or 'file', got '{store}'")));
    }
    if !ref_exists(db, kind, ref_id)? {
        return Err(attachment_error("NOT_FOUND", format!("{kind} {ref_id} does not exist")));
    }
    let meta = std::fs::metadata(file_path).map_err(|_| attachment_error("ATTACHMENT_FILE_NOT_FOUND", format!("file '{file_path}' does not exist")))?;
    if !meta.is_file() {
        return Err(attachment_error("ATTACHMENT_FILE_NOT_FOUND", format!("'{file_path}' is not a file")));
    }
    let size = meta.len() as usize;
    if size > MAX_ATTACHMENT_BYTES {
        return Err(attachment_error("ATTACHMENT_TOO_LARGE", format!("file is {size} bytes — the cap is {MAX_ATTACHMENT_BYTES} (25 MB)")));
    }
    if size == 0 {
        return Err(attachment_error("ATTACHMENT_EMPTY", "file is empty — nothing to attach"));
    }
    let bytes = std::fs::read(file_path).map_err(|e| BukioError::new("IO_ERROR", e.to_string()))?;
    let sha256 = format!("{:x}", Sha256::digest(&bytes));
    let dup = db.query_row("SELECT id FROM attachments WHERE kind = ?1 AND ref_id = ?2 AND sha256 = ?3",
        rusqlite::params![kind, ref_id, sha256], |r| r.get::<_, i64>(0));
    if let Ok(id) = dup {
        return Err(attachment_error("ATTACHMENT_DUPLICATE", format!("the same file is already attached (attachment {id})")));
    }
    ...
    if dry_run {
        return Ok(json!({"action": "attachments.add", "kind": kind, "ref_id": ref_id, "file_name": file_name, "mime": mime, "size": size, "sha256": sha256, "mode": store, "dryRun": true}));
    }
    ...
    record(db, RecordArgs { actor, action: "attachments.add", command: Some("attach add"), args: Some(json!({"attachment_id": id, "kind": kind, "ref_id": ref_id, "file_name": file_name, "size": size, "mode": store, "sha256": sha256})), outcome: "ok", entry_ids: vec![] })?;
    Ok(json!({"id": id, "kind": kind, "ref_id": ref_id, "file_name": file_name, "mime": mime, "size": size, "sha256": sha256, "mode": store, "note": note, "created_by": actor}))
}
...
pub fn get_attachment(db: &Connection, id: i64) -> Result<Value> {
    ...
    match row { Ok(v) => Ok(v), Err(rusqlite::Error::QueryReturnedNoRows) => Err(attachment_error("ATTACHMENT_NOT_FOUND", format!("attachment {id} does not exist"))), Err(e) => Err(sql_err(e)) }
}
...
pub fn remove_attachment(db: &Connection, id: i64, actor: &str, dry_run: bool) -> Result<Value> {
    let row = db.query_row(
        ...
        [id], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, String>(1)?, r.get::<_, i64>(2)?, r.get::<_, String>(3)?, r.get::<_, String>(4)?, r.get::<_, Option<String>>(5)?)),
    ).map_err(|_| attachment_error("ATTACHMENT_NOT_FOUND", format!("attachment {id} does not exist")))?;
    if dry_run {
        return Ok(json!({"action": "attachments.remove", "id": id, "kind": row.1, "ref_id": row.2, "file_name": row.3, "mode": row.4, "dryRun": true}));
    }
    db.execute("DELETE FROM attachments WHERE id = ?1", [id]).map_err(sql_err)?;
    record(db, RecordArgs { actor, action: "attachments.remove", command: Some("attach remove"), args: Some(json!({"attachment_id": id, "kind": row.1, "ref_id": row.2, "file_name": row.3, "mode": row.4})), outcome: "ok", entry_ids: vec![] })?;
...
mod tests {
    ...
    fn test_db() -> Connection {
        ...
        d.execute("INSERT INTO company (name) VALUES ('AttachCo')", []).unwrap();
    ...
    fn add_and_list_attachment() {
        ...
        let result = add_attachment(&d, "entry", 1, &path, None, "db", "human:erik", false).unwrap();
        assert_eq!(result["kind"], "entry");
        assert_eq!(result["size"], 11);
        let list = list_attachments(&d, "entry", 1).unwrap();
        assert_eq!(list.len(), 1);
        assert_eq!(list[0]["file_name"].as_str().unwrap().contains("test_attach"), true);
    ...
    fn duplicate_rejected() {
        ...
        let err = add_attachment(&d, "entry", 1, &path, None, "db", "human:erik", false).unwrap_err();
