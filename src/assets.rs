...
fn assets_error(code: &'static str, msg: impl Into<String>) -> BukioError { BukioError::new(code, msg.into()) }
fn valid_date(s: &str) -> bool { chrono::NaiveDate::parse_from_str(s, "%Y-%m-%d").is_ok() }

fn month_diff(a: &str, b: &str) -> i32 {
    let (y1, m1) = (a[..4].parse::<i32>().unwrap(), a[5..7].parse::<i32>().unwrap());
    let (y2, m2) = (b[..4].parse::<i32>().unwrap(), b[5..7].parse::<i32>().unwrap());
    y2 * 12 + m2 - (y1 * 12 + m1)
}
...
fn add_months(date_str: &str, n: i32) -> String {
    ...
    let last_day = chrono::NaiveDate::from_ymd_opt(yy, mm + 1, 1)
        .map(|d| (d - chrono::Duration::days(1)).format("%d").to_string().parse::<u32>().unwrap_or(28))
        .unwrap_or(28);
...
fn first_run_period(recognition: &str, start: &str) -> String {
    let eff = if recognition > start { recognition } else { start };
    let day: u32 = eff[8..10].parse().unwrap_or(1);
    let month = &eff[..7];
    if day == 1 { month.to_string() } else { next_period(month) }
}
...
pub fn get_scheme(db: &Connection, id: i64) -> Result<Option<Value>> {
    ...
    match r { Ok(v) => Ok(Some(v)), Err(rusqlite::Error::QueryReturnedNoRows) => Ok(None), Err(e) => Err(assets_error("DB_ERROR", e.to_string())) }
}

pub fn ensure_default_scheme(db: &Connection, actor: &str) -> Result<Value> {
    let existing = db.query_row("SELECT id FROM depreciation_schemes WHERE name = 'Standaard 5 jaar lineair'", [], |r| r.get::<_, i64>(0));
    if let Ok(id) = existing { return get_scheme(db, id)?.ok_or_else(|| assets_error("DB_ERROR", "scheme not found")); }
    create_scheme(db, "Standaard 5 jaar lineair", "lineair", 60, 0, actor, false)
}

pub fn create_scheme(db: &Connection, name: &str, method: &str, life_months: i64, residual_bp: i64, actor: &str, dry_run: bool) -> Result<Value> {
    if name.trim().is_empty() { return Err(assets_error("INVALID_NAME", "scheme needs a name")); }
    if !["lineair", "degressief"].contains(&method) { return Err(assets_error("INVALID_METHOD", format!("method must be lineair or degressief, got '{method}'"))); }
    if life_months < 1 || life_months > 600 { return Err(assets_error("INVALID_LIFE", "life-months must be 1-600")); }
    if residual_bp < 0 || residual_bp > 10000 { return Err(assets_error("INVALID_RESIDUAL", "residual-bp must be 0-10000")); }
    if dry_run { return Ok(json!({"action": "assets.scheme.add", "name": name, "method": method, "life_months": life_months, "residual_bp": residual_bp, "dryRun": true})); }
...
pub fn schedule_depreciation(cost_cents: i64, residual_cents: i64, life_months: i64, method: &str, first_period: &str, as_of: &str) -> Vec<(String, i64)> {
    let depreciable = cost_cents - residual_cents;
    if depreciable <= 0 || first_period.is_empty() || as_of.is_empty() || as_of < first_period { return vec![]; }
    let rate = if method == "degressief" { 2.0 / life_months as f64 } else { 0.0 };
    let mut out = Vec::new();
    ...
    while p.as_str() <= as_of && remaining > 0 && guard < 2000 {
        ...
        let deg = if method == "degressief" { ((remaining as f64) * rate).round() as i64 } else { 0 };
        let mut amt = if method == "degressief" { std::cmp::max(linear, deg) } else { linear };
...
fn serialize_asset(db: &Connection, row: &rusqlite::Row<'_>) -> rusqlite::Result<Value> {
    let id: i64 = row.get(0)?; let name: String = row.get(1)?;
    let status: String = row.get(4)?; let scheme_id: i64 = row.get(5)?;
    let purchase_date: String = row.get(6)?; let purchase_price_cents: i64 = row.get(7)?;
    let residual_cents: i64 = row.get(8)?; let dep_start: String = row.get(9)?;
    let recog: String = row.get(10)?; let cum_dep_at_rec: i64 = row.get(11)?;
    let asset_acct_id: i64 = row.get(12)?; let cum_dep_acct_id: Option<i64> = row.get(13)?;
    let expense_acct_id: i64 = row.get(14)?;
    let asset_acct = db.query_row("SELECT code FROM accounts WHERE id = ?1", [asset_acct_id], |r| r.get::<_, String>(0)).ok();
    let cum_dep_acct = cum_dep_acct_id.and_then(|id| db.query_row("SELECT code FROM accounts WHERE id = ?1", [id], |r| r.get::<_, String>(0)).ok());
    let expense_acct = db.query_row("SELECT code FROM accounts WHERE id = ?1", [expense_acct_id], |r| r.get::<_, String>(0)).ok();
    let scheme = get_scheme(db, scheme_id).ok().flatten();
    Ok(json!({"id": id, "name": name, "scheme": scheme, "purchase_date": purchase_date, "purchase_price_cents": purchase_price_cents, "residual_cents": residual_cents, "depreciation_start_date": dep_start, "recognition_date": recog, "cum_dep_at_recognition_cents": cum_dep_at_rec, "asset_account_code": asset_acct.as_deref(), "cum_dep_account_code": cum_dep_acct.as_deref(), "expense_account_code": expense_acct.as_deref(), "status": status}))
}

pub fn get_asset(db: &Connection, id: i64) -> Result<Option<Value>> {
    let r = db.query_row("SELECT * FROM assets WHERE id = ?1", [id], |r| serialize_asset(db, r));
    match r { Ok(v) => Ok(Some(v)), Err(rusqlite::Error::QueryReturnedNoRows) => Ok(None), Err(e) => Err(assets_error("DB_ERROR", e.to_string())) }
}

pub fn list_assets(db: &Connection, status: Option<&str>) -> Result<Vec<Value>> {
    if let Some(s) = status {
        let mut stmt = db.prepare("SELECT * FROM assets WHERE status = ?1 ORDER BY id").map_err(sql_err)?;
        let rows = stmt.query_map([s], |r| serialize_asset(db, r)).map_err(sql_err)?;
        Ok(rows.filter_map(|r| r.ok()).collect())
    } else {
        let mut stmt = db.prepare("SELECT * FROM assets ORDER BY id").map_err(sql_err)?;
        let rows = stmt.query_map([], |r| serialize_asset(db, r)).map_err(sql_err)?;
        Ok(rows.filter_map(|r| r.ok()).collect())
    }
}
...
pub fn run_due(db: &Connection, period: &str, actor: &str, dry_run: bool) -> Result<Value> {
    ...
    let target = if period.is_empty() { chrono::Utc::now().format("%Y-%m").to_string() } else { period.to_string() };
    let mut plan = Vec::new(); let mut to_book = Vec::new();
    for a in &assets {
        ...
        let cum = a["cum_dep_account_code"].as_str().or(a["asset_account_code"].as_str()).unwrap_or("");
        ...
        let booked: std::collections::HashSet<String> = db.prepare("SELECT period FROM asset_depreciation_runs WHERE asset_id = ?1")
            .map_err(sql_err)?.query_map([aid], |r| r.get::<_, String>(0)).map_err(sql_err)?
            .filter_map(|r| r.ok()).collect();
        let due: Vec<_> = sched.into_iter().filter(|(p, _)| !booked.contains(p)).collect();
        if !due.is_empty() {
    ...
    if to_book.is_empty() { return Ok(json!({"booked": [], "plan": plan, "dryRun": dry_run})); }
    if dry_run { return Ok(json!({"booked": [], "plan": plan, "dryRun": true})); }
    ...
    for (a, due, exp, cum) in &to_book {
        ...
        for (period, amt) in due {
            ...
            let entry = create_entry(db, CreateEntry { date: &format!("{period}-01"), description: &desc, postings: vec![PostingSpec { code: exp.clone(), amount_cents: *amt, cost_center_code: None }, PostingSpec { code: cum.clone(), amount_cents: -amt, cost_center_code: None }], source: "assets", source_ref: Some(&sr), actor })?;
    ...
    record(db, RecordArgs { actor, action: "assets.run", command: Some("assets run"), args: Some(json!({"period": target})), outcome: "ok", entry_ids: booked.iter().filter_map(|b| b["entry_id"].as_i64()).collect() })?;
