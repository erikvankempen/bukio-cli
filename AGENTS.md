...
| `INVALID_COUNTRY` / `COUNTRY_NOT_SUPPORTED` / `PROFILE_NOT_FOUND` | `init --country` / jurisdiction-profile resolution: not an ISO 3166-1 alpha-2 code, a valid country with no profile implemented yet (Phase A supports NL; GB/US/FR/LU planned), or no profile for a valid code | Use a 2-letter code; supported: NL |
