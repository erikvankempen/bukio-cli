...
| `bukio attach add --invoice N\|--entry N --file F [--store db\|file] [--note] [--dry-run]` / `attach list --invoice N\|--entry N` / `attach show --id N [--out F]` / `attach remove --id N` | Store source documents against invoices/entries. Default `--store db` = BLOB in the DB (travels with backups); `--store file` = copy in `<db>-attachments/` with the path stored. Lists are metadata-only. |
...
| `ATTACHMENT_FILE_NOT_FOUND` / `ATTACHMENT_TOO_LARGE` / `ATTACHMENT_NOT_FOUND` / `ATTACHMENT_DUPLICATE` / `ATTACHMENT_FILE_MISSING` / `REF_REQUIRED` / `FILE_EXISTS` / `INVALID_STORE` | `attach` problems: file missing, > 25 MB, unknown id, exact duplicate, file-mode copy missing on disk, neither/`--entry` nor `--invoice` given, `--out` target exists (needs `--force`), or `--store` not `db`/`file` | Use `attach list` to find ids; pass exactly one ref; pass `--force` to overwrite; `--store db` (default, travels with backups) or `file` |
