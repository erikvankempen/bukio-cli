...
| `PEPPOL_NOT_CONFIGURED` / `PEPPOL_SEND_FAILED` / `PEPPOL_BUYER_MISSING_ID` | No Peppol endpoint, the provider rejected/unreachable, or the buyer has no KVK number (BT-49 requires it) | Set `BUKIO_PEPPOL_ENDPOINT`/`BUKIO_PEPPOL_TOKEN`; check the provider response; set the buyer KVK via `contact update --id <id> --kvk <number>` |
