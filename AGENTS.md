...
| `bukio vat file --period 2026-Q2 [--account 2510] [--dry-run]` | Reclassify the outstanding VAT position (clears 1500/2500) to 'Af te dragen omzetbelasting' (default 2510, auto-created) at filing. Exact cents — the form is filed rounded. |
| `bukio vat settle --tx <id> [--period] [--difference-account 4700] [--dry-run]` | Book the bank payment that cancels the af-te-dragen balance (tx must be unmatched; outgoing for te betalen, incoming for a refund). The whole-euro filing vs exact-cents liability rounding difference goes to the P&L difference account (default 4700). Atomic: entry + tx reconciliation commit together. |
...
| `VAT_NOTHING_TO_FILE` / `VAT_SETTLE_NOTHING` | `vat file`: 1500/2500 net is zero (nothing to reclassify) / `vat settle`: 2510 af te dragen balance is zero (nothing filed or already settled) | Check the VAT readout + `report trial-balance` for 1500/2500/2510 before filing/settling |
