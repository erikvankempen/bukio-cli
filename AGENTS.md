...
This file is the **agent's manual** for bukio-cli. Read it before driving the tool. It assumes you have shell access to the machine where bukio-cli is installed and that `bukio` is on PATH.
...
| `bukio invoice pdf --id N` / `ubl --id N` / `credit --id N` / `pay --id N --date` | PDF (native renderer), Peppol BIS 3.0 XML, credit notes, payments. |
...
| `NOT_FINALIZED` / `CONTACT_NOT_FOUND` / `INVOICE_REF_REQUIRED` / `PDF_UNAVAILABLE` | Invoice not finalized, unknown contact, missing invoice reference, or the PDF could not be written | Finalize first; fix the contact; `PDF_UNAVAILABLE` means the PDF could not be written (empty output or an unwritable path) — check the `--out` directory and permissions |
...
> Every code above is emitted by at least one command. This table is the authoritative code list.
...
`GIT_ERROR`, `SERVER_START` — version control, MCP server start.
...
- ❌ Forgetting to enable VAT (`vat enable`) before using `vat book` or expecting VAT accounts in the core chart — VAT is an optional module.
