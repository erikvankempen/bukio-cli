...
| `ALREADY_ENROLLED` / `INVALID_KEY` / `NOT_ENROLLED` / `ALREADY_REVOKED` / `INVALID_REASON` | Actor-key registry: key already active, missing keyid/public key, actor not enrolled, already revoked, or revocation without a reason | `actor register` a fresh key after `actor revoke` (rotation); always pass a revocation reason |
